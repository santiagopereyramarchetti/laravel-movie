        ...
        <main v-if="season" class="my-2">
            <section class="bg-gradient-to-r from-indigo-600">
                <div class="max-w-6xl mx-auto m-4 p-2">
                    <div class="flex">
                        <div class="w-3/12">
                            <div class="w-full">
                                <img class="w-full h-full rounded" :src="`https://www.themoviedb.org/t/p/w220_and_h330_face/${season.poster_path}`">
                            </div>
                        </div>
                        <div class="w-8/12">
                            <div class="m-4 p-6">
                                ...
                                <div class="flex p-3 text-white space-x-4">
                                    <span>{{ tvShow.name }} - {{ tvShow.created_year }} - Season {{ season.season_number }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <section class="max-w-6xl mx-auto bg-gray-200 dark:bg-gray-900 p-2 rounded">
                <div class="flex justify-between">
                    <div class="w-7/12">
                        <h1 class="flex text-gray-500 font-bold text-xl">Episodes</h1>
                        <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mt-4">
                            
                            <MovieCard v-for="episode in season.episodes" :key="season.id">
                                <template #image>
                                    <Link :href="route('season.show', [season.slug, season.slug])">
